...
const NutritionMeter = () => {
  ...
  useEffect(() => {
    ...
    if (calculateTotalCalories >= 3000) {
      setShowWarning(true);
    } else {
  ...
  const totalProtein = () => {
    return parseFloat(nutritionItems.reduce(
      (total, item) => total + parseFloat(item.protein) * item.quantity, 0
    )).toFixed(2);
  };

  const totalCarbs = () => {
    return nutritionItems.reduce(
      (total, item) => total + parseFloat(item.carbs) * item.quantity, 0
    );
  };

  const totalFat = () => {
    return nutritionItems.reduce(
      (total, item) => total + parseFloat(item.fat) * item.quantity, 0
    );
  };

  return (
    <div className="bg-white mt-3 min-h-screen">
      <div className="container mx-auto p-4">
        <h1 className="text-3xl font-semibold text-center mb-4"> Nutrition Tracker </h1>
        {!showWarning && (
          <div className="bg-orange-600 text-white p-2 rounded-md text-center mb-4">
            <FontAwesomeIcon icon={faTimes} className="mr-2" />
            Total calories are below the recommended limit (3000 calories) !
          </div>
        )}
